...
# set permissions and ownership for /etc/cron.allow:
...
# create /etc/at.allow:
touch /etc/at.allow
# set permissions and ownership for /etc/at.allow:
...
echo "Ciphers chacha20-poly1305@openssh.com,aes256-gcm@openssh.com,aes128-
gcm@openssh.com,aes256-ctr,aes192-ctr,aes128-ctr" >> /etc/ssh/sshd_config
#5.2.13 Ensure only strong MAC algorithms are used (Automated)
echo "MACs hmac-sha2-512-etm@openssh.com,hmac-sha2-256-etm@openssh.com,hmac-sha2-
512,hmac-sha2-256" >> /etc/ssh/sshd_config
#5.2.14 Ensure only strong Key Exchange algorithms are used (Automated)
echo "KexAlgorithms curve25519-sha256,curve25519-sha256@libssh.org,diffie-hellmangroup14-sha256,diffie-hellman-group16-sha512,diffie-hellman-group18-
sha512,ecdh-sha2-nistp521,ecdh-sha2-nistp384,ecdh-sha2-nistp256,diffiehellman-group-exchange-sha256" >>  /etc/ssh/sshd_config
...
DenyUsers <userlist>
DenyGroups <grouplist>" >> /etc/ssh/sshd_config
